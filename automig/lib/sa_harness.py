...
def render_col(col_name, col_type, details, composite_pkey):
  ...
  return f"sa.Column('{col_name}', {TYPES[col_type]}{', ' + ', '.join('%s=%s' % pair for pair in details.items()) if details else ''})"
  ...
  for tablename, stmts in diffing.group_by_table(table_stmts).items():
    ...
    for stmt in stmts:
      if isinstance(stmt, wrappers.CreateTable):
        ...
        table_strings.append('\n'.join([f"class {stmt.name}Enum(enum.Enum):"] + [f'\t{val} = {i + 1}' for i, val in enumerate(stmt.values)]))
